...
type CloudinaryImg = { width: number; height: number; src: string; format?: string };

export const getCloudinaryImgUrl = ({ width, height, src, format = "webp" }: CloudinaryImg) => {
  const extendedSrc = src.replace("upload/", `upload/w_${width},h_${height},c_fill,q_auto/`);

  return { originalUrl: extendedSrc, formattedUrl: changeImgFormat({ src: extendedSrc, format }) };
};
